...
export default function NewFacilityPage() {
    ...
    const handleChange = (section: keyof Facility, field: string | null, value: any) => {
        setFormData(prev => {
            if (field === null) {
                // Updating top-level field (hotel, visa, food, medical)
                return { ...prev, [section]: value };
            } else if (section === 'transport' && field === 'status') {
                // Initialize routes if switching to Yes
                ...
                // Updating nested field for others
                return {
                    ...prev,
                    [section]: {
                        ...(prev[section] as any),
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            await ApiClient.post('/facilities', formData);
            router.push('/facilities');
        } catch (err: any) {
            console.error("Submission error:", err);
            // Display more detailed error if possible
            const errorMessage = err.message || (typeof err === 'string' ? err : JSON.stringify(err));
            setError(errorMessage || "Failed to create facility record");
        } finally {
            setLoading(false);
        }
    };

    // Helper to render Yes/No Radio Group
    ...
    return (
        <div className="max-w-3xl mx-auto py-6">
            <h1 className="text-2xl font-bold text-gray-900 mb-6">New Facility Record</h1>
            ...
            <form onSubmit={handleSubmit} className="bg-white shadow rounded-lg p-6 space-y-6">
                    ...
                    {formData.ticket.status === 'Yes' && (
                        <div className="mt-4 bg-gray-50 p-4 rounded-md">
                            ...
                            <select
                                required
                                className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm p-2"
                                value={formData.ticket.ticket_type}
                    ...
                    {formData.umrahs.status === 'Yes' && (
                        <div className="mt-4 bg-gray-50 p-4 rounded-md">
                            ...
                            <input
                                ...
                                required
                                className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm p-2"
                                value={formData.umrahs.umrahs_count}
                ...
                <div className="flex justify-end pt-6">
                    ...
                    >
                        {loading ? 'Saving...' : 'Save Facility Record'}
                    </button>
                </div>
            </form>
        </div>
